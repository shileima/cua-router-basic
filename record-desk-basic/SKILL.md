...
4. 优先用稳定语义接口（连接器/专用工具）完成稳定操作；UI 交互、依赖视觉的校验、或“操作界面本身就是任务”时用 **Computer Use**。每步「定位 → 交互 → 校验」，关键步骤后用 `ax.get` 确认结果。
5. 生成的回放技能应**依赖 cua-router-basic**、用 `sky.*` + `ax.*` 执行，跑在已内置的快执行链上——这是比坐标回放更准、更快、更抗漂移的方式。模板见 `references/replay-skill-template.md`。
6. 按 `skill-creator` 规范创建**可发现的真实技能**并完成结构校验，而不只是一份 Markdown runbook。
7. 完成后给用户一段简明总结：**逐条对应录制动作的完整步骤清单 + 输入参数 + 假设**，便于复核纠正。
